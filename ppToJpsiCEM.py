...
def qqbarToccbar(s,m,alphaS):
     w=(4.0*m**2)/s
     return (8.0*np.pi*alphaS**2)/(27.0*s)*(1 + w/2.0)*np.sqrt(1.0-w)
...
def epToJpsi_CEM(x, flag, sqs, mc, y):
        ...
        x_g1=np.sqrt(M2)*np.exp(y)*(1/sqs)  
        x_g2=np.sqrt(M2)*np.exp(-y)*(1/sqs)  

        if (0 < x_g1 < 1 and 0 < x_g2 < 1):

            s=sqs*sqs
            glx_g1 = p.xfxQ2(21,x_g1,M2)/x_g1
            glx_g2 = p.xfxQ2(21,x_g2,M2)/x_g2   
            alphaS = p.alphasQ2(M2)
            energy = sqs/2

            if  (flag=="D"):
                return (2.0/s)*qT*glx_g1*glx_g2*TMDpart("D", qT)*ggToccbar(M2,mc,alphaS)
            elif  (flag=="N"):
                return (1.0/s)*qT*glx_g1*2*Sivers_xpart(x_g1)*glx_g2*TMDpart("N", qT)*ggToccbar(M2,mc,alphaS)*np.sin((np.pi/2.0)-phi_qT)*np.sin(phi_qT-(np.pi/2.0))
            else:
                print('Invalid flag!')
