...
def epToJpsi_ICEM(x, flag, sqs, mc):
            y=x[0]
            phi_qT= x[1] 
            M2 = x[2]

            ms=np.sqrt(M2)/MJpsi #Momentum shift 
            
            x_g=ms*np.sqrt(M2)*np.exp(y)*(1/sqs)  
            x_p=ms*np.sqrt(M2)*np.exp(-y)*(1/sqs)  
            ...
            if (0 < x_g < 1):
                ...
                if  (flag=="D"):
                    return (2.0/s)*qTP*glx_g*WWZ(x_p, sqs, M2)*TMDpart("D", qTP)*ggToccbar(M2,mc,alphaS)
                elif  (flag=="N"):
                    return (1.0/s)*qTP*glx_g*2*Sivers_xpart(x_g)*WWZ(x_p, sqs, M2)*TMDpart("N", qTP)*ggToccbar(M2,mc,alphaS)*np.sin((np.pi/2.0)-phi_qT)*np.sin(phi_qT-(np.pi/2.0))
                else:
                    print('Invalid flag!')
        
#main
#Energy= [4.7,7.2,31.6,158.1]
sqs= 158.1
mc = 1.27
qT = 0.1
mD = 1.87
def denominator(x):
    return epToJpsi_ICEM(x, "D", sqs, mc)
def numerator(x):
    return epToJpsi_ICEM(x, "N", sqs, mc)
...
while (qT < 1.5):
    integrator = vegas.Integrator([y_limits, phi_qT_limits, M2_limits], nproc=8)
    ...
    print(qT, Asym)
    data.append([qT, Asym])
    with open (f"data/icem_qT1_{sqs}.dat",'w') as file:
