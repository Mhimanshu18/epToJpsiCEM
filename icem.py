...
def epToJpsi_ICEM(x, flag, sqs, mc, y):
            qT=x[0]
            phi_qT= x[1] 
            M2 = x[2]
...
def numerator(x):
   return epToJpsi_ICEM(x, "N", sqs, mc, y)

qT_limits = [0.0, 1.0]
phi_qT_limits = [0.0, 2.0*np.pi]
M2_limits = [MJpsi**2, 4*mD**2]
data=[]
...
while (y < 1.3):
        ...
        data.append([y, Asym])
        with open (f"data/icem_y_{sqs}.dat",'w') as file:
