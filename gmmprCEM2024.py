...
def epToJpsi_CEM(x, flag, sqs, mc, qT2):
        z=x[0]
        phi_qT= x[1] 
        M2 = x[2]
        MJpsi=3.096
        ...
        if (0 < x_g < 1):
            ...
            if  (flag=="D"):
                return (1.0/(4*z*s))*glx_g*fgammae(x_p, energy)*TMDpart("D", qT)*ggToccbar(M2,mc,alphaS)
            elif  (flag=="N"):
                return (1.0/s)*qT*glx_g*2*Sivers_xpart(x_g)*fgammae(x_p, energy)*TMDpart("N", qT)*ggToccbar(M2,mc,alphaS)\
                            *np.sin((np.pi/2.0)-phi_qT)*np.sin(phi_qT-(np.pi/2.0))
            else:
                print('Invalid flag!')
...
def denominator(x):
    return epToJpsi_CEM(x, "D", sqs, mc, qT2)

def numerator(x):
    return epToJpsi_CEM(x, "N", sqs, mc, qT2)
...
"""data.append([qT2, result_den.mean,result_num.mean])
